// ==UserScript==
// @name         Bunkr Sort & Filter Buttons Combined Updated + Gallery A/D | ←/→ | MouseWheel | Infinite Scroll | Cache (IndexedDB)
// @version      1.0
...
(function () {
    ...
    (function initMainFeatures() {
        ...
        function showLoadingIndicator(total) {
            ...
            loader.style = `position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(0,0,0,0.9); padding: 20px 40px; border-radius: 10px; color: white; z-index: 99999; font-size: 16px; text-align: center;`;
        ...
        function initFilterMenu() {
            ...
            style.innerHTML = ` ::-webkit-input-placeholder { color: rgb(167, 139, 250) !important; } `;
    ...
    (function initNavigation() {
        ...
        // Rechtsklick erkennung über das contextmneu
        document.addEventListener('contextmenu', function (e) {
            // 1. Klicks auf Eingabefelder, Buttons etc. ignorieren
            if (e.target.closest('button, a, input, textarea')) return;

            // 2. Prüfen, ob die Galerie WIRKLICH offen ist.
            // LightGallery setzt 'lg-on' auf den Body und 'lg-visible' auf ihren äußeren Container.
            const isGalleryOpen = document.body.classList.contains('lg-on') ||
                  document.querySelector('.lg-outer.lg-visible') !== null;

            // 3. Nur blockieren und schließen, wenn die Galerie tatsächlich aktiv ist
            if (isGalleryOpen && window.lgInstance) {
                e.preventDefault();
    ...
    (function initGallerySorter() {
        ...
        function jumpToImage(inputEl) {
            ...
            if (window.lgInstance && !isNaN(index)) {
        ...
        function initGalleryFeature(galleryBtn) {
            ...
            newBtn.addEventListener("click", async function(e) {
                ...
                try {
                    ...
                    if (window.lgInstance) window.lgInstance.destroy(true);
